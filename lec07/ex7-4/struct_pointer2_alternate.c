/*
    Alternatively, we can also pass the pointer to the whole array
    and select the correct member inside the print function. Since
    we are only passing the pointer and not the struct, we aren't
    copying all the data in the elements, so this doesn't incur a
